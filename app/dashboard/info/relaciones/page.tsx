...
export default async function RelacionesPage({
  ...
  const records = await getRecords(query, 'relaciones');

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex-1">
          <Search placeholder="Buscar en relaciones institucionales..." />
        </div>
        <Button asChild>
          <Link href="/dashboard/info/relaciones?action=new">
            <PlusCircle className="mr-2" />
            Añadir proyecto o relación
          </Link>
        </Button>
      </div>
