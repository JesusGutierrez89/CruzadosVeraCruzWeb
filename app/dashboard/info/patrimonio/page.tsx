...
export default async function PatrimonioPage({
  ...
  const records = await getRecords(query, 'patrimonio');

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex-1">
          <Search placeholder="Buscar en patrimonio..." />
        </div>
        <Button asChild>
          <Link href="/dashboard/info/patrimonio?action=new">
            <PlusCircle className="mr-2" />
            Añadir información patrimonial
          </Link>
        </Button>
      </div>
